...
class OpenSearchClient:
    def __init__(self, emb, index_name, mapping_name, vector, text, output):
        ...
        self.emb = emb
        self.config = config
        self.endpoint = pm.get_params(key="opensearch_domain_endpoint", enc=False)
        #self.endpoint = f"{domain_endpoint}"
        self.http_auth = (pm.get_params(key="opensearch_user_id", enc=False), pm.get_params(key="opensearch_user_password", enc=True))
        self.vector = vector
        self.text = text
        self.output = output
